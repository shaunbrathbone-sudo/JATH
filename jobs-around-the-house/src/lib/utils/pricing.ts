...
const calculatePrice = (
    ...
    for (const rule of perUnitRules) {
        ...
        if (qty > 0 && rate > 0) {
            ...
            modifiers.push({
                label:
                    rule.description ||
                    `${rule.fieldKey}: ${qty} × £${rate.toFixed(2)}`,
                amount,
                type: "add",
            });
        }
        ...
        try {
            ...
            if (matches) {
                if (rule.rate) {
                    ...
                    modifiers.push({
                        label:
                            rule.description || `${cond.field}: ${cond.value}`,
                        amount: rule.rate,
                        type: "add",
                    });
                }
                if (rule.multiplier) {
                    ...
                    modifiers.push({
                        label:
                            rule.description ||
                            `${cond.value} (×${rule.multiplier.toFixed(2)})`,
        ...
        if (answerVal !== undefined && rule.multiplier) {
            ...
            if (shouldApply) {
                ...
                modifiers.push({
                    label:
                        rule.description ||
                        `${rule.fieldKey} (×${rule.multiplier.toFixed(2)})`,
                    amount: rule.multiplier,
                    type: "multiply",
                });
            }
    ...
    const depositPct =
        getGlobalVar(globalVariables, "deposit_percentage", 50) / 100;
