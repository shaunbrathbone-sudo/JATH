...
export default function ProductImageManager({ product }: Props) {
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      if (res.ok) {
        setMessage({ text: "Image uploaded and set as active!", type: "success" });
        router.refresh();
      } else {
  ...
  // We can reuse the same toggle route since it operates on the image ID directly
  ...
  const handleSetBackground = async (url: string | null) => {
    setUpdatingBg(product.id);
    setMessage(null);
    ...
    try {
      ...
      if (res.ok) {
        setMessage({
          text: url ? "Main featured image set successfully!" : "Featured image cleared.",
          type: "success"
        });
        router.refresh();
      } else {
        const data = await res.json();
        setMessage({ text: data.error || "Failed to update featured image", type: "error" });
      }
    } catch {
      setMessage({ text: "Network error", type: "error" });
    } finally {
  ...
  // We reuse the same DELETE route because it operates on the image ID directly
  ...
  const activeBg = product.imageUrl || product.heroImages.find((img) => img.isActive)?.imageUrl || "";
  const hasImages = product.heroImages.length > 0;
  const limitReached = product.heroImages.length >= 6;

  return (
    <div className="hero-manager">
      <div className="hero-manager__item">
        <div className="hero-manager__preview">
          {activeBg ? (
            <img
              src={activeBg}
              alt={`Active image preview for ${product.name}`}
        ...
        <div className="hero-manager__info">
          <div className="hero-manager__header">
            <div>
              <h3 className="hero-manager__name">{product.name} Images</h3>
              <p className="hero-manager__slug">
                {product.heroImages.length}/6 images uploaded
                {product.imageUrl ? " · Custom Featured Image Set" : ""}
              </p>
            </div>
          </div>
          ...
          {hasImages && (
            <div className="hero-manager__grid">
              {product.heroImages.map((img) => {
                const isBg = product.imageUrl === img.imageUrl;
                return (
                  <div key={img.id} className={`hero-manager__thumb-card ${isBg ? "hero-manager__thumb-card--is-bg" : ""}`}>
                    ...
                    <div className="hero-manager__thumb-controls">
                      ...
                      <button
                        ...
                        onClick={() => handleSetBackground(isBg ? null : img.imageUrl)}
                        disabled={updatingBg === product.id}
                        title={isBg ? "Remove as featured image" : "Set as featured image"}
                      >
          ...
          <div className="hero-manager__actions">
            ...
            <button
              ...
              disabled={uploading === product.id || limitReached}
              onClick={() => fileInputRef.current?.click()}
            >
              {uploading === product.id ? "Uploading..." : limitReached ? "Limit Reached (6/6)" : "Add Image"}
            </button>
          </div>
        </div>
      </div>
