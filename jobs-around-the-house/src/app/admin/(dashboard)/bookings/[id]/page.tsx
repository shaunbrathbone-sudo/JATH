      ...
      <div className="admin-detail-grid">
        ...
        <div className="admin-detail-main">
          ...
          <div className="admin-card">
            ...
            {booking.items.map((item) => (
              <div key={item.id} className="admin-booking-item">
                <div className="admin-booking-item__header">
                  <h3>{item.product.name}</h3>
                  <span className="admin-amount">£{item.calculatedPrice.toFixed(2)}</span>
                </div>
                {item.configs.length > 0 && (
                  <div className="admin-booking-item__configs">
                    {item.configs.map((config) => (
                      <div key={config.id} className="admin-config">
                        <span className="admin-config__key">{config.fieldKey.replace(/_/g, " ")}</span>
                        <span className="admin-config__value">{config.fieldValue}</span>
                      </div>
                    ))}
                  </div>
                )}
          ...
          {booking.customerNotes && (
            <div className="admin-card">
              <h2 className="admin-card__title">Customer Notes</h2>
              <p className="admin-notes">{booking.customerNotes}</p>
            </div>
          )}
        </div>
        ...
        <div className="admin-detail-sidebar">
          <div className="admin-card">
            ...
            <div className="admin-detail-list">
              ...
              <div className="admin-detail-list__item">
                ...
                <a href={`mailto:${booking.customer.email}`} className="admin-detail-list__value admin-detail-list__link">
              ...
              {booking.customer.phone && (
                <div className="admin-detail-list__item">
                  ...
                  <a href={`tel:${booking.customer.phone}`} className="admin-detail-list__value admin-detail-list__link">
                    {booking.customer.phone}
                  </a>
                </div>
              )}
              {booking.customer.addressLine1 && (
                <div className="admin-detail-list__item">
                  ...
                  <span className="admin-detail-list__value">
                    {booking.customer.addressLine1}
                    {booking.customer.addressLine2 && <br />}
                    {booking.customer.addressLine2}
                    {booking.customer.city && <><br />{booking.customer.city}</>}
                    {booking.customer.postcode && <><br />{booking.customer.postcode}</>}
                  </span>
                </div>
              )}
            </div>
          ...
          <div className="admin-card">
            ...
            <div className="admin-detail-list">
              <div className="admin-detail-list__item">
                <span className="admin-detail-list__label">Preferred Date</span>
                <span className="admin-detail-list__value">
              ...
              <div className="admin-detail-list__item">
                <span className="admin-detail-list__label">Preferred Time</span>
                <span className="admin-detail-list__value">
                  {booking.preferredTime || "No preference"}
                </span>
              </div>
              <div className="admin-detail-list__item">
                <span className="admin-detail-list__label">Booked</span>
                <span className="admin-detail-list__value">
